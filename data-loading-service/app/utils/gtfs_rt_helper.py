...
def update_gtfs_realtime_data():
    ...
    for agency in SWIFTLY_AGENCY_IDS:
        ...
        stop_time_df = pd.DataFrame(stop_time_array).astype({'arrival': 'int8', 'departure': 'int8', 'stop_sequence': 'int8', 'schedule_relationship': 'int8','timestamp': 'unint64'})
        del stop_time_array
        ...
        trip_update_df = pd.DataFrame(trip_update_array).astype({'direction_id': 'int8', 'schedule_relationship': 'int8', 'timestamp': 'unint64'})
        ...
        vehicle_position_updates = pd.DataFrame(vehicle_position_update_array).astype({'current_stop_sequence': 'int8', 'current_status': 'int8', 'timestamp': 'unint64'})
